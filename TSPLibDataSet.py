...
class TSPLibDataSet(Dataset):
    def __init__(self, filename, seq_len):
        ...
        self.opt_cost = np.zeros(len(opt_data), dtype=float)
        for k in range(len(opt_data)):
            ...
            for i in range(0,count_opt_tour):
                ...
                count = count + 1
            self.opt_cost[k] = opt_data[k].TourCost
        self.data[:,:,0] = self.data[:,:,0]-1  

        self.masked_output = np.ones((self.data.shape[0],orig_len-1,orig_len))
        for i in range(0,self.data.shape[0]): 
            for j in range(0,orig_len-1):
                for k in range(0,j):
    ...
    def __getitem__(self, index):
        return torch.tensor(self.data[index]).float().cuda(),torch.tensor(self.masked_output[index]), torch.tensor(self.opt_cost[index]).cuda()
...
def main():
    fname = "data/TSPTrainingData_Test_for_KroA100_100.txt"
    t1 = TSPLibDataSet(100,fname)
